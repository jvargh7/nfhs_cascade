...
bind_rows(unmet_svysummary_dm,
  ...
  left_join(readxl::read_excel("data/NFHS Cascade Variable List.xlsx",sheet="map2020_v024") %>% 
            ...
            by=c("state"="v024")) %>% 
  write_csv(.,file = "cutoff200/ncc200a05_state unmet need care cascade.csv")
...
bind_rows(unmet_svysummary_dm,
  ...
  left_join(readxl::read_excel("data/NFHS Cascade Variable List.xlsx",sheet="map2020_v024") %>% 
              ...
              distinct(v024,n5_state,.keep_all=TRUE),
            by=c("state"="v024")) %>% 
  write_csv(.,file = "cutoff200/ncc200a05_state met need care cascade.csv")

# df <- read_csv(file="cutoff200/ncc200a05_state unmet need care cascade.csv") %>%
#   dplyr::select(-n5_state,-contains("zone")) %>%
#   left_join(readxl::read_excel("data/NFHS Cascade Variable List.xlsx",sheet="map2020_v024") %>%
#               dplyr::select(v024,n5_state,zone) %>%
#               distinct(v024,n5_state,.keep_all=TRUE),
#             by=c("state"="v024"))
# write_csv(df,file = "cutoff200/ncc200a05_state unmet need care cascade.csv")
