...
nfhs5dmdiag_df <- bind_rows(readRDS(paste0(path_cascade_folder,"/working/nfhs5 iapr_women pre3.RDS")) %>% 
                              mutate(sex = "Female"),
                            readRDS(paste0(path_cascade_folder,"/working/nfhs5 iapr_men pre3.RDS")) %>% 
                              mutate(sex = "Male")) %>%
