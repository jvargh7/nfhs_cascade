...
# 2. Use rpg_cutoff3 = 160 mg/dL cutoff 



ncp_preprocessing3 <- function(df, sex = "Female"){
  ...
  mutate(screened_dm = case_when(screened_dm == 1 ~ 1,
    ...
    mutate(fasting = case_when(lastate > 94 | lastdrank > 94 ~ NA_real_,
           ...
           dm = case_when(diagnosed_dm == 1 ~ 1,
                          ...
                          fasting == 0 & glucose >= rpg_cutoff3 ~ 1,
                          is.na(fasting) & glucose >= rpg_cutoff3 ~ 1,
                          fasting == 1 & glucose < fpg_cutoff ~ 0,
                          fasting == 0 & glucose < rpg_cutoff3 ~ 0,
                          is.na(fasting) & glucose < rpg_cutoff3 ~ 0,
                          TRUE  ~ NA_real_),
  ...
  mutate(
    ...
    dm_diag_untreat = case_when(diagnosed_dm == 1 & treated_dm == 1 ~ 0,
                                diagnosed_dm == 1 & treated_dm == 0 ~ 1,
                                TRUE ~ NA_real_),
    
    # Dignosis: Yes, Treated: Yes, Blood sugar: out of control range
    dm_treat_uncontr = case_when(treated_dm == 0 | is.na(treated_dm)  ~ NA_real_,
                                 treated_dm == 1 & diagdm == 1 ~ 1,
                                 treated_dm == 1 & diagdm == 0 ~ 0,
                                 TRUE ~ NA_real_),
  ...
  mutate(prediabetes = case_when(diagnosed_dm == 1 ~ NA_real_,
                                 ...
                                 fasting == 0 & glucose >= rpgpre_cutoff & glucose < rpg_cutoff3~ 1,
                                 is.na(fasting) & glucose >= rpgpre_cutoff & glucose < rpg_cutoff3 ~ 1,
