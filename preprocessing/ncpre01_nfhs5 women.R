...
mutate_at(vars(shb18s,shb18d,
  ...
  mutate(bmi_underweight = case_when(ha40 > 6000 ~ NA_real_,
                                       ha40 < 1850 ~ 1,
                                       ha40 >= 1850 ~ 0,
                                       TRUE ~ NA_real_),
         
         
         bmi_overweight = case_when(ha40 > 6000 ~ NA_real_,
                                      ha40 >= 2500 & ha40 < 3000 ~ 1,
                                      ha40 < 2500 | ha40 >= 3000 ~ 0,
                                      TRUE ~ NA_real_),
         
         
         bmi_obese = case_when(ha40 > 6000 ~ NA_real_,
                                 ha40 >= 3000 ~ 1,
                                 ha40 < 3000 ~ 0,
                                 TRUE ~ NA_real_)) %>% 
  ...
  mutate(fasting = case_when(shb53 > 94 | shb54 > 94 ~ NA_real_,
         ...
         highglucose = case_when(
                                   is.na(shb74) | shb74 > 498 ~ NA_real_,
                                   fasting == 1 & shb74 >= fpg_cutoff ~ 1,
                                   fasting == 0 & shb74 >= rpg_cutoff ~ 1,
                                   is.na(fasting) & shb74 >= rpg_cutoff ~ 1,
                                   fasting == 1 & shb74 < fpg_cutoff ~ 0,
                                   fasting == 0 & shb74 < rpg_cutoff ~ 0,
                                   is.na(fasting) & shb74 < rpg_cutoff ~ 0,
                                   TRUE  ~ NA_real_),

         diagdm = case_when(
           is.na(shb74) | shb74 > 498 ~ NA_real_,
           selfdiabetes == 1 & fasting == 1 & shb74 >= fpg_cutoff ~ 1,
           selfdiabetes == 1 & fasting == 0 & shb74 >= rpg_cutoff ~ 1,
           selfdiabetes == 1 & is.na(fasting) & shb74 >= rpg_cutoff ~ 1,
           selfdiabetes == 1 & fasting == 1 & shb74 < fpg_cutoff ~ 0,
           selfdiabetes == 1 & fasting == 0 & shb74 < rpg_cutoff ~ 0,
           selfdiabetes == 1 & is.na(fasting) & shb74 < rpg_cutoff ~ 0,
           TRUE  ~ NA_real_
         )
  ...
  mutate(sbp = rowMeans(.[,c("shb18s","shb25s","shb29s")],na.rm=TRUE),
         ...
         diaghtn = case_when(
           is.na(sbp) | is.na(dbp) ~ NA_real_,
           selfhypertension == 1 & sbp >= sbp_cutoff ~ 1,
           selfhypertension == 1 & dbp >= dbp_cutoff ~ 1,
           selfhypertension == 1 & sbp < sbp_cutoff ~ 0,
           selfhypertension == 1 & dbp < dbp_cutoff ~ 0,
           TRUE ~ NA_real_),
         ) %>% 
