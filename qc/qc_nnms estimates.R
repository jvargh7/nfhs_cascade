...
nnmspop_df <- bind_rows(readRDS(paste0(path_cascade_folder,"/working/nfhs5 iapr_women.RDS")) %>% 
                          mutate(sex = "Female"),
                        readRDS(paste0(path_cascade_folder,"/working/nfhs5 iapr_men.RDS")) %>% 
                          mutate(sex = "Male")) %>%
  dplyr::filter(!is.na(dm_free),age %in% c(18:69)) %>%
...
nnms_national <- map_dfr(group_vars,
                         function(g_v){
                           id_vars = c(g_v);
                           p();
                           n5_sy <- svysummary(nnmspop_design,
...
nnms_regional <- map_dfr(group_vars,
                         function(g_v){
...
nnms_dm_national <- map_dfr(group_vars,
                            function(g_v){
                              id_vars = c(g_v);
                              p();
                              n5_sy <- svysummary(nnmspop_dm_design,
                                                  # c_vars = continuous_vars,
...
nnms_dm_regional <- map_dfr(group_vars,
                            function(g_v){
                              id_vars = c("residence",g_v);
                              p();
                              n5_sy <- svysummary(nnmspop_dm_design,
...
write_csv(nnms_national,"qc/nnms national equivalent estimates.csv")
write_csv(nnms_regional,"qc/nnms regional equivalent estimates.csv")
write_csv(nnms_dm_national,"qc/nnms national equivalent nested estimates.csv")
write_csv(nnms_dm_regional,"qc/nnms regional equivalent nested estimates.csv")
