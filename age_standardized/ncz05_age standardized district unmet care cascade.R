...
require(furrr)
options(future.globals.maxSize= (4*1024*1024)^2) #6GB
...
bind_rows(unmet_svysummary_dm,
  ...
  left_join(readxl::read_excel("data/NFHS Cascade Variable List.xlsx","map2018_sdist") %>% 
            ...
            by=c("D_CODE")) %>% 
  write_csv(.,file = "age_standardized/ncz05_district unmet need care cascade.csv")
...
bind_rows(unmet_svysummary_dm,
  ...
  left_join(readxl::read_excel("data/NFHS Cascade Variable List.xlsx","map2018_sdist") %>% 
              ...
              mutate(D_CODE = sprintf("%03d",as.numeric(D_CODE))),
            by=c("D_CODE")) %>% 
  write_csv(.,file = "age_standardized/ncz05_district met need care cascade.csv")
