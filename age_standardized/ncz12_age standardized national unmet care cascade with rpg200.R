...
unmet_n5_sy <- bind_rows(
  
  svysummary(nfhs5dmz_svydesign,
                    # c_vars = continuous_vars,
                    p_vars = proportion_vars[1:4],
                    # g_vars = grouped_vars,
                    # id_vars = id_vars
),
svysummary(nfhs5dmdiagz_svydesign,
           # c_vars = continuous_vars,
...
unmet_n5_sy %>% 
  write_csv(.,file = "age_standardized/ncz12_total age standardized national unmet and met care cascade with rpg200.csv")
...
bind_rows(
  ...
  write_csv(.,file = "age_standardized/ncz12_national unmet need care cascade with rpg200.csv")
...
bind_rows(unmet_svysummary_dm,
  ...
  write_csv(.,file = "age_standardized/ncz12_national met need care cascade with rpg200.csv")
