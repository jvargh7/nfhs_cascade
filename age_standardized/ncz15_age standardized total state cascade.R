...
state_svysummary %>% 
  left_join(readxl::read_excel("data/NFHS Cascade Variable List.xlsx",sheet="map2020_v024") %>% 
            ...
            by=c("state"="v024")) %>% 
  write_csv(.,path = "age_standardized/ncz15_age standardized state cascade.csv")


# df <- read_csv(file="age_standardized/ncz15_age standardized state cascade.csv") %>%
#   left_join(readxl::read_excel("data/NFHS Cascade Variable List.xlsx",sheet="map2020_v024") %>%
#               dplyr::select(v024,n5_state,zone) %>%
#               distinct(v024,n5_state,.keep_all=TRUE),
#             by=c("state"="v024"))
# write_csv(df,file = "age_standardized/ncz15_age standardized state cascade.csv")
