...
figA <- nfhs5_women %>% 
  ...
  ggplot(data=,aes(x=f_age,y=f_dm_treated*100,group=phase,col = phase)) +
...
figC <- nfhs5_women %>% 
  dplyr::filter(hv025 == "Urban") %>% 
  ggplot(data=,aes(x=f_age,y=f_htn_treated*100,group=phase,col = phase)) +
  coord_cartesian(ylim =c(0,100)) +
  geom_smooth(method="lm",formula=y ~ ns(x,4),fill="grey80") +
  theme_bw() +
  xlab("Age in years") +
...
ggarrange(figA,figB,figC,figD,
  ...
  ggsave(.,filename = paste0(path_cascade_folder,"/figures/age trends treated by urban rural.png"))
